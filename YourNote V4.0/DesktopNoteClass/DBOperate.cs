...
namespace DesktopNote
{
    class DBOperate
    {
        ...
        public int OperateData(string strSql)
        {
            ...
            if (i > 0)
            {
                MessageBox.Show("执行成功！", "恭喜", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
        ...
        public int OperateData1(string strSql)
        {
            if (conn.State == ConnectionState.Closed)
                conn.Open();
            SqlCommand cmd = new SqlCommand(strSql, conn);
        ...
        public DataSet GetTable1(string sql, string dirName, DataGridView DGDir)
        {//返回dataset
        ...
        //执行双击选中的单元格
        public void DoubleClick(string sql, RichTextBox rtb, DataGridViewCellEventArgs e)
        {
            if(conn.State==ConnectionState.Closed)
                conn.Open();
            ...
            while (dr.Read())
            {
                rtb.Text = (string)dr.GetValue(e.ColumnIndex);
            }
            dr.Close();
            if(conn.State==ConnectionState.Open)
                conn.Close();
        ...
        public int GetNoteCount(string sql,DataGridViewCellEventArgs ee)
        {
            if (conn.State == ConnectionState.Closed)
                conn.Open();
            SqlCommand cmd = new SqlCommand(sql, conn);
            ...
            while (dr.Read())
            {
                count= (int)dr.GetValue(ee.ColumnIndex);
            }
            dr.Close();
            if (conn.State == ConnectionState.Open)
                conn.Close();
